...
\ctitle{基于文字边缘骨架切割和文本行定位矫正的场景文字检测}
...
为了更准确的定位到文字，本文接着提出一种基于二叉树搜索的文本行定位矫正方法。首先对于输入的场景文字图片，利用基于边缘骨架切割的文字检测子来提取粗略的候选文字包围盒定位结果。然而其定位结果存在重叠率不足的问题，为了进一步提高文字检测的精度，首先对候选文字包围盒计算统计边缘响应，接着通过对求取到的统计边缘响应进行水平投影、求取梯度以及执行非极大值抑制等操作，以获得候选文本行。最后由生成的候选文本行中建立起二叉树型的搜索空间，并根据优化策略从搜索空间中找到最优的文本行定位结果。该结果即是经过优化后得到的文字细致定位结果，相比于边缘骨架切割文字检测子的粗略定位结果而言，该方法的文字检测精度得到了大幅提升。
...
In order to locate the text more accurately, this paper then proposes a text line rectification method based on binary tree search. First of all, for the input scene text image, the rough candidate text bounding box results are extracted by skeleton-cut detector. However, the overlap rate is not meet the requirement. In order to further improve the accuracy of text detection, we first calculate the static skeleton-box response form the candidate text bounding boxes , and then the candidate text lines are obtained form the response by horizontal projection, gradient and non maximum suppression operation. Finally, the binary-tree-based search space is built from candidate text lines, and the optimal result is found from the search space according to the optimization strategy.

The two methods proposed in this paper are compared with the related methods on ICDAR2011, ICDAR2013 and MSRA-TD500 test set respectively, and the experimental results prove the robustness of our methods. In addition, this paper also get the results of the proposed methods on the more challenging SVT dataset, which proves the practicability and reliability of the algorithm.
