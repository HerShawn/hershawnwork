...
首先针对边缘粘连问题，本文提出了一种基于边缘骨架切割的文字检测方法。该方法首先利用结构化边缘检测方法得到边缘响应图，然后在每个二值边缘图上通过细化操作得到其边缘骨架图，并通过8领域内像素点分析所找到的边缘骨架结点。接着断开粘连点以获得候选文字边缘骨架，再经过形态学滤除来过滤掉大部分明显不是文字的边缘骨架，剩余的不易区分的非文字边缘骨架，可通过基于CNN的分类器来进一步滤除。最后基于非极大值抑制和文本行聚集操作，得到文本行定位结果。
