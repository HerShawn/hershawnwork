...
In order to locate the text more accurately, this paper then proposes a text detection method based on binary tree search. First of all, for the input scene text image, the rough candidate text bounding box results are extracted by skeleton-cut detector. However, the overlap rate is not meet the requirement. In order to further improve the accuracy of text detection, we first calculate the static skeleton-box response form the candidate text bounding boxes , and then the candidate text lines are obtained form the response by horizontal projection, gradient and non maximum suppression operation. Finally, the binary-tree-based search space is built from candidate text lines, and the optimal result is found from the search space according to the optimization strategy.
