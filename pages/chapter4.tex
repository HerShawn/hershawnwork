...
\echapter{Text detection based on Binary Tree Search}
    ...
    针对上述问题，本章提出一种基于二叉树搜索的文本行定位优化框架，用来调整候选文字包围盒与ground truth 的重叠率以提升文字检测和识别效果。该框架包含3个步骤：第1步是对所有的候选文字包围盒进行统计边缘响应（Static Skeleton-box Response，简称SSR 算法），然后对计算得到的SSR求取梯度并执行NMS操作，以获得候选文本行；第2步是将候选文本行的所有可能的排列组合方式构建成一个二叉树型搜索空间；第3步是仿照二叉树的后续遍历算法从搜索空间中找到一条最优路径，该路径对应着优化后的文字定位包围盒。值得注意的是在搜索最优文字定位的过程中还利用到了文字识别的结果，而经过该算法调优后的文字定位结果又进一步提升识别文字的精度。本章接下来内容会详细论述这一优化框架。
    ...
    \esection{Candidate text line's extraction}
        ...
        首先提出统计边缘响应方法，用以增强文本与背景之间的响应强度差异。给定如图\ref{fig.c4_static_skeleton_response}(a) 所示的场景文字图像$g$，计算其统计边缘响应$s$ 的方式如图\ref{fig.c4_ser}中算法2 所示：

        %算法\ref{alg:c4_static_skeleton_response} 所示：
        ...
        \begin{figure*}[!h]
        \centering
        \includegraphics[width=\textwidth]{c4_ser.jpg}
        \caption{统计边缘响应} \label{fig.c4_ser}
        \end{figure*}
    ...
    在明确候选文本行分割线$L$，以及二叉树型搜索空间$N$ 中各变量的含义后，由$L$ 构建$N$ 的过程如图\ref{fig.c4_build_binary_tree}中的算法3所示：

    %算法\ref{alg:c4_build_binary_tree} 所示：
   ...
   \begin{figure*}[!h]
   \centering
   \includegraphics[width=\textwidth]{c4_build_binary_tree.jpg}
   \caption{递归地建立二叉树型搜索空间} \label{fig.c4_build_binary_tree}
   \end{figure*}

    那么，通过算法3从$L$ 中构建好的二叉树型搜索空间$N$就将存储在如图中的表格内。下一节，将介绍从搜索空间$N$ 中搜寻出最优文本行定位的搜索策略。
    ...
    \esection{Searching the optimal text detection}
        ...
        给定二叉树型搜索空间$N$,假设$N$是由$m$ 个结点$n$ 构成，那么通过图\ref{fig.c4_algori_bts}中的算法4，就可搜索出路径$P={p_1,p_2,...,p_{(m-1)/2}}$。 路径中的每个$p_i \in P$ 都是1个3元组：包含了父亲结点$np_i$，左子结点$nl_i$ 和右子结点$nr_i$。
        ...
        为了更直观地理解算法4：从二叉树型搜索空间中找到最优路径的算法，这里用图\ref{fig.c4_bianry_tree_search} 所示的例子加以阐述。首先图\ref{fig.c4_bianry_tree_search}(a) 中的表格存储的是代表候选文本行搜索空间的二叉树$N$，且在这个例子中，$N$含有7 个结点；然后采用算法4在$N$ 上进行搜索，算法将会执行$(7-1)/2=3$ 次迭代，每次迭代的具体过程如图\ref{fig.c4_bianry_tree_search}(b) 所示；最后，算法的每次迭代，都会生成1 个对应的结点三元组。那么经历3 次迭代后，就会得到如图\ref{fig.c4_bianry_tree_search}(c) 所示的路径集合，即为算法4的搜索结果。
