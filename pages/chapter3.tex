...
\echapter{Text detection based on Skeleton-cut detector}
    ...
    \esection{Text skeleton's extraction and cutting}
        ...
        细化方法将重复迭代操作，直到提取出的边缘的骨架图不再变化为止。因此候选文字骨架的生成步骤如图\ref{fig.c3_skeleton}所示：首先利用结构化边缘检测子在输入的场景文字图像中计算得到结构化边缘响应，然后通过一系列分割阈值获得相应的二值边缘图，最后通过8连通区域细化方法得到边缘骨架图，如图\ref{fig.c3_skeleton}(d)所示
