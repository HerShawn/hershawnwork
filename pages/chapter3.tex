...
\label{sec.c3}
    ...
    本文上述所提出的基于边缘骨架切割的检测方法和2 阶段分类判别方法可以获得较高的文字检测查全率，但是文字检测结果同真实结果值(ground truth)之间的重叠率还是不满足期望。为了达到文字检测过程中的紧凑性(compactness) 标准，本文通过如图\ref{fig.c3_algori_ilr}所示的算法1来调整文字检测包围框，使其更靠近文字的真实位置：

    %下面的算法\ref{alg:c3_iter_local_refine} 来调整文字检测包围框，使其更靠近文字的真实位置：
   ...
   \begin{figure*}[!h]
   \centering
   \includegraphics[width=\textwidth]{c3_algori_ilr.jpg}
   \caption{文本行的局部迭代优化算法} \label{fig.c3_algori_ilr}
   \end{figure*}
